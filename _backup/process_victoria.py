...
def process_victoria_data():
    # 读取事件数据
    event_df = pd.read_csv('data/Victoria/Victoria_Event_Schedule.csv')
    ...
    # 读取BAS数据
    bas_files = glob.glob('data/Victoria/BAS/**/*.csv', recursive=True)
    bas_dfs = []
    ...
    # 读取WBEL数据
    wbel_files = glob.glob('data/Victoria/WBEL/**/*.csv', recursive=True)
    wbel_dfs = []
    ...
    for column in bas_data.columns:
        if column != 'timestamp':
            # 移除列名中的 VICTORIA 前缀（包括可能的空格）
            clean_column = column.replace('VICTORIA_', '').replace('VICTORIA ', '')
    ...
    # 保存处理后的数据
    output_path = 'data/Victoria/Victoria_5min_merged.csv'
...
if __name__ == "__main__":
    process_victoria_data()

# 删除缺失值过多的时间点数量: 213232
# 删除前的数据行数: 274752
# 删除后的数据行数: 61520
# 处理完成，数据已保存至: data/Victoria/Victoria_5min_merged.csv
# 需求响应时间点数: 1170
# 总时间点数: 61520
# 需求响应时间点占比: 1.90%